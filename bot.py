...
async def help(ctx):
  ...
  embed = embed.add_field(name=f"{prefix}findplayers", value="to get matching players of a server", inline=False)
...
async def tds(ctx):
  if ctx.author.id == 936924519032049704:
    ...
    embed = discord.Embed(title="Total Discord Servers", description=ts_list, colour=ctx.bot.user.colour)
...
async def imginfo(ctx,*,server:str):
  ...
  for a in soup.find_all('a', href=True):
    ...
    if str(server.lower()) in str((a.get_text()).lower()):
      ...
      imgresult = requests.get(f"http://www.game-state.com/{a['href']}/banner", headers=headers)
      imgsoup = BeautifulSoup(imgresult.text, 'html.parser')

      imgtext =  imgsoup.find(id='preview_560x95')
      imgurl = str(imgtext)
      await msg.delete()
      await ctx.send(f"https://www.game-state.com/{imgurl[43:-3]}")
...
# --- findplayers
@bot.command()
async def findplayers(ctx, playername:str, *,server:str):
  ...
  for a in soup.find_all('a', href=True):
    ...
    if str(server.lower()) in str((a.get_text().lower())):
      ...
      for player in playerslist.find_all('td'):
        ...
        if str(playername.lower()) in str((player.get_text()).lower()):
      ...
      if tp <= 0:
        await msg.delete()
        return await ctx.send(f"There are no matching player online with name `{playername}`")


      await msg.delete()
      embed = discord.Embed(title=f"{str(a.get_text())}", description=player_list, colour = ctx.bot.user.colour)
      embed = embed.add_field(name="Matching Player", value=tp, inline=False)
      embed = embed.set_footer(text="Big Smoke", icon_url=ctx.bot.user.avatar_url)
      return await ctx.send(embed=embed)
...
async def info_error(ctx, error):
  if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.BadArgument):
    await ctx.send(f"{ctx.author.mention} Correct Usage: `{prefix}info [Server Name]`")

@findplayers.error
async def findplayers_error(ctx, error):
  if isinstance(error, commands.MissingRequiredArgument) or isinstance(error, commands.BadArgument):
    await ctx.send(f"{ctx.author.mention} Correct Usage: `{prefix}findplayers [Player_Name] [Server Name]`")
